...
async function fetchSites(): Promise<ProjectType[]> {
  ...
  const VercelProjects = await Promise.all(
    VercelSites.pagination
      ? VercelSites.projects?.map(async (project) => {
          const languages = await fetchGit(
...
const NetlifySites = () => {
  const { data: projects, error } = useSWR("projects", fetchSites, {
    ...
    <Suspense fallback={<div>Loading...</div>}>
      ...
      >
        <div className="mb-12 font-josefinSlab">
          <h2 className="text-4xl md:text-6xl font-bold text-purple-500 hover:text-purple-600 font-mono transform hover:scale-110 transition-transform duration-300">
            Portfolio!
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-16 gap-y-12">
          {undeployedProjects.map((project: ProjectType, index: number) => (
